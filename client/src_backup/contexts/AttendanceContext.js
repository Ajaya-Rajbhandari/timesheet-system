...
export const AttendanceProvider = ({ children }) => {
  ...
  const saveStatusToCache = useCallback(
    (status) => {
  ...
  const refreshStatus = useCallback(
    async (force = false) => {
      ...
      console.log(
        "Authentication check passed, proceeding with refresh for user:",
        user._id,
      );
      ...
      try {
        ...
        console.log(
          "Refreshing attendance status from server for user:",
          user._id,
        );
        ...
        try {
          ...
          if (cachedStatus) {
            ...
            console.log(
              "Using cached status due to server error:",
              parsedStatus,
            );
  ...
  const scheduleRetry = useCallback(() => {
    ...
    const backoffDelay = Math.min(
      RETRY_DELAY * Math.pow(1.5, retryAttempts.current - 1),
      30000,
    );

    console.log(
      `Scheduling retry attempt ${retryAttempts.current} in ${backoffDelay}ms`,
    );
  ...
  useEffect(() => {
    const loadInitialStatus = async () => {
      try {
        ...
        console.log(
          "User authenticated, loading initial status for user:",
          user._id,
        );
        ...
        if (cachedStatus) {
          try {
            ...
            const cachedDate = parsedStatus.lastUpdated
              ? new Date(parsedStatus.lastUpdated)
              : null;
            const today = new Date();
            const isToday =
              cachedDate && cachedDate.toDateString() === today.toDateString();
