...
function executeCommand(command, workingDir) {
  console.log(`${colors.bright}${colors.cyan}Executing:${colors.reset} ${command}`);
  try {
    execSync(command, { 
      cwd: workingDir, 
      stdio: 'inherit' 
...
`;
  ...
  console.log(`${colors.yellow}Created .env.production file. Please update with your production values.${colors.reset}`);
}

console.log(`\n${colors.bright}${colors.green}=== Deployment Preparation Complete ===${colors.reset}`);
console.log(`\n${colors.yellow}Next steps:${colors.reset}`);
console.log(`1. Update the ${colors.bright}server/.env.production${colors.reset} file with your production values`);
console.log(`2. Deploy the application to your production server`);
console.log(`3. Start the server with ${colors.bright}NODE_ENV=production node server/server.js${colors.reset}`); 