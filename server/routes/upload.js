...
router.post('/profile', auth, upload.single('profileImage'), async (req, res) => {
  try {
    ...
    if (req.body.previousImage && req.body.previousImage !== 'default-profile.png') {
      const previousImagePath = path.join(__dirname, '../uploads/profile', req.body.previousImage);
...
router.get('/profile/:filename', (req, res) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    const defaultPath = path.join(__dirname, '../uploads/profile/default-profile.png');
