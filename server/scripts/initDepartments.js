...
const defaultDepartments = [
  {
    ...
    code: "IT",
    description:
      "IT department responsible for technology infrastructure and support",
  },
  {
...
const initDepartments = async () => {
  try {
    ...
    if (existingDepartments.length > 0) {
      console.log(
        `${existingDepartments.length} departments already exist in the database.`,
      );
      ...
      readline.question(
        "Do you want to create additional default departments? (y/n) ",
        async (answer) => {
...
const createDepartments = async () => {
  try {
    ...
    if (!adminUser) {
      console.log(
        "No admin user found. Creating departments without managers.",
      );
    } else {
    ...
    for (const dept of defaultDepartments) {
      ...
      if (existingDept) {
        console.log(
          `Department ${dept.name} (${dept.code}) already exists. Skipping.`,
        );
        continue;
      }
