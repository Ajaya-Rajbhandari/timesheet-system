  ...
  try {
    ...
    if (records.length === 0) {
      ...
      for (const record of records) {
        ...
        console.log(`Clock Out: ${record.clockOut ? moment(record.clockOut).format('HH:mm:ss') : 'Not clocked out'}`);
    ...
    if (stuckRecords.length > 0) {
      ...
      for (const record of stuckRecords) {
        console.log(`\nUser: ${record.user.firstName} ${record.user.lastName} (${record.user.email})`);
        console.log(`Clock In: ${moment(record.clockIn).format('YYYY-MM-DD HH:mm:ss')}`);
        ...
        await record.save();
        console.log(`Fixed: Set clock out to ${endOfDay.format('YYYY-MM-DD HH:mm:ss')}`);
      }
    }
